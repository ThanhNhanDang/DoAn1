...
void RSAAuth::Send(char cstr[],HardwareSerial swSer)
{
  byte en[255];
  byte pac[255];
  int len=0;
...
bool RSAAuth::Receive(byte buff[255],int*BEGIN,int*END,byte m[255],int *pDesLen,HardwareSerial swSer)
{
  int i;
  //Serial.println("Nhan :");
  ...
  {    
    //Serial.println((char*)desArr);
    //Serial.println("CutString :");
    //for(i=0;i<=*pDesLen;i++)
      //Serial.println(desArr[i]);
